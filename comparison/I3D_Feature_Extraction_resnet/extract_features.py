...
def load_rgb_batch(frames_dir, rgb_files, frame_indices, transform):
	...
	for i in range(frame_indices.shape[0]):
		...
		for j in range(frame_indices.shape[1]):
			...
			img = load_frame(os.path.join(frames_dir, rgb_files[frame_indices[i][j]]))
			batch_imgs.append(img)
		# TODO fix it
		batch_data[i] = transform(batch_imgs).numpy().transpose(0, 2, 3, 1)
	return batch_data
...
def run(i3d, frequency, frames_dir, batch_size, sample_mode):
	...
	for batch_id in range(batch_num): 
		batch_data = load_rgb_batch(frames_dir, rgb_files, frame_indices[batch_id], transform)
		if(sample_mode == 'oversample'):
