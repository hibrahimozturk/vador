...
xdv_path = '../../data/object_features/train_resnet'
...
for json_path in json_paths:
    ...
    for video_name, clip_list in tqdm(anns['video_clips'].items()):
        if 'banner' in video_name or 'molotof' in video_name:
            continue
        ...
        for clip_name_crop in clip_list:
            ...
            if 'Normal' in video_name:
                clip_path = osp.join(xdv_path, 'all', video_name + '_i3d', '_'.join(clip_name_crop.split('_')[-3:]) + '.npz')
            else:
