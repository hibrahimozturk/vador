...
class Dataset(data.Dataset):
    def __init__(self, args, is_normal=True, transform=None, test_mode=False):
        ...
        elif self.dataset == 'xd-violance':
            if test_mode:
                self.rgb_list_file = 'list/xdv-i3d-test-1crop.list'
            else:
                self.rgb_list_file = 'list/xdv-i3d-test-1crop.list'
        else:
            if test_mode:
                self.rgb_list_file = 'list/ucf-i3d-test-1crop.list'
    ...
    def __getitem__(self, index):
        ...
        # file_name = osp.join('../UCF_Test_one_crop_i3d',  self.list[index].strip().split('\\')[-1])
        file_name = osp.join('../XDV_Test_one_crop_i3d',  self.list[index].strip().split('\\')[-1])
        features = np.load(file_name + '.npy', allow_pickle=True)
        # features = np.load(file_name, allow_pickle=True)

        features = np.array(features, dtype=np.float32)
        # video_name = self.list[index].strip().split('\\')[-1].rsplit('.', 1)[0]
        video_name = self.list[index].strip().split('\\')[-1].rsplit('_', 1)[0]
