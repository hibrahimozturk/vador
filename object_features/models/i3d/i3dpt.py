...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        self.avg_pool_ = torch.nn.AvgPool3d((2, 1, 1), (1, 1, 1))
    ...
    def feature_extract(self, inp, end_point='avg_pool'):
        num_frames = inp.shape[2]
        # Preprocessing
        out = self.conv3d_1a_7x7(inp)
        out = self.maxPool3d_2a_3x3(out)
        out = self.conv3d_2b_1x1(out)
        ...
        if end_point == 'mixed_5c':
            out = self.avg_pool_(out)
            return out[:, :, 0, :, :]
        if end_point == 'avg_pool':
            if num_frames == 16:
                out = self.avg_pool(out)
            else:
...
if __name__ == '__main__':
    ...
    inp = torch.rand(2, 3, 16, 224, 224)
    feat = model.feature_extract(inp)
    print('finisih')
