...
class TransformerModel(CaptionModel):
    ...
    def make_model(self, src_vocab, tgt_vocab, encoder_N=6, decoder_N=6,
        ...
        model = EncoderDecoder(
            Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout), encoder_N),
            Decoder(DecoderLayer(d_model, c(attn), c(attn),
                                 c(ff), dropout), decoder_N),
    ...
    def __init__(self, opt):
        ...
        self.model = self.make_model(
            0, tgt_vocab, encoder_N=opt.encoder_num_layers,
            decoder_N=opt.decoder_num_layers,
    ...
    def _sample_feature(self, fc_feats, att_feats, att_masks=None, opt={}):
        sample_max = opt.get('sample_max', 1)
        beam_size = opt.get('beam_size', 1)
        temperature = opt.get('temperature', 1.0)
        decoding_constraint = opt.get('decoding_constraint', 0)
